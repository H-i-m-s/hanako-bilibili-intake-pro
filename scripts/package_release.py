...
def candidate_whisper_model_paths() -> list[Path]:
    candidates: list[Path] = []

    env_cache = os.environ.get("WHISPER_CACHE_DIR")
    if env_cache:
        candidates.append(Path(env_cache) / BUNDLED_WHISPER_MODEL)

    home_cache = Path.home() / ".cache" / "whisper" / BUNDLED_WHISPER_MODEL
    candidates.append(home_cache)

    userprofile = os.environ.get("USERPROFILE")
    if userprofile:
        candidates.append(Path(userprofile) / ".cache" / "whisper" / BUNDLED_WHISPER_MODEL)
        candidates.append(Path(userprofile) / "AppData" / "Local" / "whisper" / BUNDLED_WHISPER_MODEL)
        candidates.append(Path(userprofile) / "AppData" / "Local" / ".cache" / "whisper" / BUNDLED_WHISPER_MODEL)

    for path in sorted(Path("/mnt/c/Users").glob("*/.cache/whisper/base.pt")):
        candidates.append(path)
    for path in sorted(Path("/mnt/c/Users").glob("*/AppData/Local/whisper/base.pt")):
        candidates.append(path)
    for path in sorted(Path("/mnt/c/Users").glob("*/AppData/Local/.cache/whisper/base.pt")):
        candidates.append(path)
...
def copy_bundled_whisper_model(stage_dir: Path) -> None:
    for candidate in candidate_whisper_model_paths():
        if candidate.is_file():
            target = stage_dir / "assets" / "whisper" / BUNDLED_WHISPER_MODEL
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(candidate, target)
            size_mb = target.stat().st_size / (1024 * 1024)
            log(f"bundled Whisper model from: {candidate}")
            log(f"bundled model target: {target} ({size_mb:.2f} MiB)")
            return

    searched = "\n".join(f"  - {path}" for path in candidate_whisper_model_paths())
    raise SystemExit(
        "Whisper base model was not found in any known cache location; cannot build the requested prebundled package.\n"
        f"Searched:\n{searched}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a Hanako-installable release zip that includes runtime dependencies.")
    parser.add_argument("--out-dir", default="dist", help="Output directory for the generated zip (default: dist)")
    parser.add_argument("--smoke", action="store_true", help="Run tests/smoke.mjs inside the staged release before zipping.")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=f"{plugin_name}-build-", dir=plugin_root.parent) as temp_dir:
        ...
        log("copying bundled Whisper base model into staged copy")
        copy_bundled_whisper_model(stage_dir)
